...
async def test_debug_mode_logs_everything(hass: HomeAssistant, caplog) -> None:
    import logging

    from custom_components.tapo_kasa_alarm.const import DEBUG_LOGGER_NAME

    dev = fake_device()
    entry, _ = await _setup(hass, dev)
    debug_logger = logging.getLogger(DEBUG_LOGGER_NAME)
    assert hass.states.get("switch.bahce_debug_mode").state == "off"
    assert debug_logger.level == logging.NOTSET

    caplog.clear()
    await entry.runtime_data.async_refresh()
    assert "request getAlertConfig" not in caplog.text

    await _press(hass, "turn_on", "switch.bahce_debug_mode")
    await hass.async_block_till_done()
    assert entry.options["debug"] is True
    assert entry.runtime_data.api.debug is True
    assert debug_logger.level == logging.DEBUG
    assert hass.states.get("switch.bahce_debug_mode").state == "on"

    caplog.clear()
    with caplog.at_level(logging.DEBUG, logger=DEBUG_LOGGER_NAME):
        await entry.runtime_data.async_refresh()
        await _press(hass, "turn_on", "switch.bahce_alarm")
    text = caplog.text
    assert "[Bahce 192.168.1.50]" in text
    assert "poll started" in text
    assert "request getAlertConfig, getMsgPushConfig" in text
    assert "answer after" in text
    assert "poll ok" in text
    assert "switch.bahce_alarm: turn on requested" in text
    assert "request setAlertConfig" in text
    assert "command set alarm done" in text
    assert "pw" not in text.replace("pwd", "")  # the password is never logged

    await _press(hass, "turn_off", "switch.bahce_debug_mode")
    await hass.async_block_till_done()
    assert entry.runtime_data.api.debug is False
    assert debug_logger.level == logging.NOTSET


async def test_debug_mode_kept_by_options_flow_and_restart(
    ...
    with caplog.at_level(logging.DEBUG, logger=DEBUG_LOGGER_NAME):
        entry, _ = await _setup(hass, fake_device(), options={"debug": True})
    assert entry.runtime_data.api.debug is True
    assert "connected: model=C520WS" in caplog.text

    result = await hass.config_entries.options.async_init(entry.entry_id)
    await hass.config_entries.options.async_configure(
        result["flow_id"], {"scan_interval": 5, "session_renew": 8, "discovery": False}
    )
    await hass.async_block_till_done()
    assert entry.options["debug"] is True
    assert entry.runtime_data.api.debug is True
    logging.getLogger(DEBUG_LOGGER_NAME).setLevel(logging.NOTSET)
